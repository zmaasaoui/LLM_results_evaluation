...
class BERTModelAnalyzer:
    ...
    def compare_final_metrics(self, metric_name, output_file=None):
        ...
        for vocab_size in self.vocab_sizes:
            ...
            if single_key in self.final_metrics and metric_name in self.final_metrics[single_key].columns:
                ...
                if all_key in self.final_metrics and metric_name in self.final_metrics[all_key].columns:
                    ...
                    all_values.append(0)  # Use 0 instead of None
        ...
        # Fix for the error: Filter out None values
        valid_indices = [i for i, v in enumerate(all_values) if v is not None and v != 0]
        if valid_indices:
            valid_x = [x[i] + width/2 for i in valid_indices]
            valid_all_values = [all_values[i] for i in valid_indices]
            all_bars = plt.bar(valid_x, valid_all_values, width, label='All Datasets', color='#ff7f0e')
